<script lang="ts">
  ...
  $: findValuesKey = () => {
    ...
    return $result.data.fields.find((f) => f.name === "value")?.name ?? $result.data.fields[0]?.name;
  }
  $: findLabelsKey = () => {
    ...
    return $result.data.fields.find((f) => f.name === "label")?.name ?? findValuesKey();
  }

  $: valuesKey = findValuesKey();
  $: labelsKey = findLabelsKey();

  $: userDefinedItems = options?.map((item) => {
  ...
  $: resultItems = $resultArray.map((item) => ({ value: item[valuesKey ?? 0], label: String(item[labelsKey ?? 0]) }));
