...
describe("GET endpoint", () => {
  ...
  it("should return JSON response for successful query execution", async () => {
    const queryResult = { toJSON: () => JSON.stringify({ data: "result" }) };
    ...
    expect(await response.json()).toEqual(JSON.stringify({ data: "result" }));
  });
  ...
  it("should return 404 status if the query file is not found", async () => {
    (findQueryFile as Mock).mockRejectedValue(
      new Error("Query file not found")
    );
    const response = await GET({
      params: { query: "testQuery" },
      url: new URL("http://localhost"),
