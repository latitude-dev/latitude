...
describe('format', () => {
  ...
  it('formats Date objects correctly', () => {
    ...
    expect(format(date)).toBe(`$date:${encodeURIComponent(richDate.toString())}:${encodeURIComponent(richDate.format)}`);
  });
  ...
  it('formats RichDate objects correctly', () => {
    ...
    expect(format(absoluteDate)).toBe(`$date:${encodeURIComponent(absoluteDate.toString())}:${encodeURIComponent(absoluteDate.format)}`);
    expect(format(relativeDate)).toBe(`$date:${encodeURIComponent(relativeDate.toString())}:${encodeURIComponent(relativeDate.format)}`);
  });
...
describe('parse', () => {
  ...
  it('parses Date objects correctly', () => {
    ...
    expect(parse(`$date:${encodeURIComponent(dateString)}:${encodeURIComponent(format)}`)).toEqual(absoluteDate);
    expect(parse(`$date:${encodeURIComponent(RelativeDate.Today)}:${encodeURIComponent(format)}`)).toEqual(relativeDate);
  });
