...
function getCodeFrame(source: string, line: number, startColumn: number, endColumn: number|undefined): string {
  ...
  return lines
    ...
    .map((str, i) => {
      ...
      if (isErrorLine) {
        const indicator =
          ' '.repeat(digits + 2 + tabsToSpaces(str.slice(0, startColumn)).length) + '^' + '~'.repeat(endColumn ? Math.max(0, endColumn - startColumn - 1) : 0);
