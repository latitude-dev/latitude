...
export abstract class BaseConnector {
   ...
   */
  protected abstract popParams(): SequentialCompiledParams | KeyBasedCompiledParams
  ...
  protected compile(sql: string, params?: QueryParams): string {
    if (params && Object.keys(params).some(key => key.startsWith(this.PHANTOM_PARAM_PREFIX))) {
      throw new SyntaxError(`Parameter keys cannot start with ${this.PHANTOM_PARAM_PREFIX}`, { query: sql })
    }
    ...
    const parseValue = (value: unknown): string => {
      ...
      const phantomParamKey = `${this.PHANTOM_PARAM_PREFIX}${Object.keys(phantomParams).length}`
    ...
    function resolveNodeExpression(node: Node, localScope: Scope): unknown {
      switch (node.type) {
        ...
        case 'Identifier':
          if (!localScope.hasOwnProperty(node.name)) throw newSyntaxError(sql, node, `Undefined variable: ${node.name}`);
          return localScope[node.name];
  
        case 'ObjectExpression':
          return node.properties.reduce((acc, prop) => {
            if (prop.type !== 'Property') throw newSyntaxError(sql, node, 'Object definition can only contain properties');
        ...
        case 'LogicalExpression':
          ...
          if (!OPERATOR_METHODS.hasOwnProperty(operator)) throw newSyntaxError(sql, node, `Unsupported operator: ${operator}`);
        ...
        default:
          throw newSyntaxError(sql, node, `Unsupported expression type: ${node.type}`);
      }
    }
    ...
    function parseNode(node: TemplateNode, localScope: Scope = {}): string {
      ...
      switch (node.type) {  
        ...
        case 'EachBlock':
          ...
          if (!Array.isArray(iterableElement)) throw newSyntaxError(sql, node.expression, 'Element in #each block must be an array')
        ...
        case 'ObjectExpression':
          throw newSyntaxError(sql, node, 'Object definitions are not allowed for interpolation')
        ...
        case 'CallExpression':
          ...
          throw newSyntaxError(sql, node.callee, `Unsupported function: ${methodName}`);
    ...
    function parseChildrenNodes(children: TemplateNode[] | undefined, localScope: Scope = {}): string {
      return children?.map((child) => {
        if (child.type === 'ConstTag') {
          ...
          if (child.expression.type !== 'AssignmentExpression') throw newSyntaxError(sql, child.expression, 'Constant definitions must assign a value')
          if (child.expression.operator !== '=') throw newSyntaxError(sql, child.expression, 'Constant definitions must use the = operator')
          if (child.expression.left.type !== 'Identifier') throw newSyntaxError(sql, child.expression, 'Constant definitions must have an identifier on the left side')
...
const newSyntaxError = (sql: string, node: BaseNodeWithoutComments, message: string) => {
