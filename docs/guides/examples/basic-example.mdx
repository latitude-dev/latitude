...
For this guide, we'll use the CSV that comes with the sample project, but you can replace that with any other CSV or even connect to a live database following [these instructions](/sources).
...
This query will select all the rows from the `source.csv` data source.
