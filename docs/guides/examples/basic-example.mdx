...
Now, create a new file named 'source.yml' and write the following:

```yaml
type: duckdb
```

This query will select all the rows from the `source.csv` file using the DuckDB adapter that you configured in the `source.yml` file.

Next, let's create a secondary query to aggregate some data. Create a new file
named `sample_agg.sql` in the `queries` directory and write the following
query:
