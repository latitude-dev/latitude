...
##  How It Works
 ...
 - **Configurable Expiration**
    ...
    This can be configured by setting the `ttl` parameter either in the [source configuration](/sources/additional-config) or [in the query itself](/queries/advanced/config).
   
 ## Forcing Refresh With Query Param

You can use the `__force` parameter to force a refresh of results on a specific page. 

See the page [parameters](/queries/logic/parameters) to learn how query params work.