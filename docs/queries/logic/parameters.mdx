...
Each parameter configuration is represented in the URL allowing you to point to a specific data result.

Imagine two inputs with the parameters `start_year` and `end_year`, their configuration will be automatically added to the URL as:

```
yourapp.com/?start_year=$text:2000&end_year=$text:2024
```

The parameters start with `?` and you can concat multiple of them with `&`.

### Force Cache Refresh

There is a parameter type `__force` that allows you to invalidate the cache results and get a fresh version.

See the [Cache page](/queries/basics/cache) to learn how it works.
