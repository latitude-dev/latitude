...
## Parameters
`param()` retrieves the value from an input and inserts it into the SQL code to generate results dynamically. 
...
<Input param="input_param_name" />

<LineChart
  query="data"
  inlineParams={{ 
    query_param_name: input("input_param_name")
  }}
/>
```
