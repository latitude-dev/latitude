...
export default async function run(watch: boolean = false) {
  ...
  if (!activeFile || activeFile?.languageId !== 'latitudeSql') {
    vscode.window.showErrorMessage(`Could not run query: File is not a Latitude query.`);
    return;
  }

  const sourceDir = getProjectSource(activeFile!.uri);
  if (!sourceDir) {
    vscode.window.showErrorMessage(`Could not find Latitude project source directory.`);
    return;
  }
  ...
  const relativePath = path.relative(queriesDir, activeFile!.uri.fsPath).replace(/\.sql$/, '');

  const terminal = vscode.window.createTerminal(`Latitude query: ${relativePath}`);
